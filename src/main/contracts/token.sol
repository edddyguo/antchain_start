...
/**
 * @dev 转移资产从一个指定账户到自己的账户 必须得到账户的授权
...
* @dev 发行资产的方法，只有管理员有权限操作
    */
function issue(identity account, uint256 value)public returns (bool) {
...
//myTransfer
function myTransfer(identity from, identity to, uint256 value) public returns(bool){
require(from != identity(0), "Asset: transfer from the zero identity");
require(to != identity(0), "Asset: transfer to the zero identity");
